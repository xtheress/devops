...
- 900 900 900
    ...
    И допустим нужно писать данные или читать данные на что-то медленное. Система не тормозит, но из-за того, что процессы записи чтения на медленный девайс скопились - ЛА подросла.  В этом случае будет высокая wait
    При этом все остальные процессы отрабатывают быстро
    
    На параметр нагрузки LA влияет также и ожидание ввода-вывода (параметр *wait*
     в утилите *top*
...
- Ответ
    ...
    D - непрерывное ожидание, сон. Ждем сигнал от аппаратной части. Нельзя прервать
...
- Ответ
    ...
    **oom_score** корректируется коэффициентом **oom_score_adj** (или oom_adj), который можно вручную настраивать для защиты или повышения приоритета завершения
...
- Ответ
    ...
    **Inode** (индексный дескриптор) - структура данных, в которой хранятся метаданные файла или каталоге и перечислены блоки с данными файла. 
    
    df -i - что-бы узнать кол-во

    Хранит всю информацию, кроме имени файла и данных.
  
    Вся информация о файле извлекается из таблицы индексов путем ссылки на номер индекса. 
...
возникнуть описанная ситуация?

- Ответ
    1. Место действительно закончилось просто некоторые файловые системы резервируют 5% свободного места под root
    2. Бывает так, что превышается квота
